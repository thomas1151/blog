...
const DATA_SOURCES = {
  POSTS: join(process.cwd(), '_blog'),
  COMPONENTS: join(process.cwd(), '_components'),
  EXPERIENCES: join(process.cwd(), '_experiences')
}
...
export const API = {
  ...
  ALL: {
    ...
    BLOG: (fields = [], limit) => returnRecordsData('BLOG', fields, limit).sort((post1, post2) => (post1.date > post2.date ? '-1' : '1')),
